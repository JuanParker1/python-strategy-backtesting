...
def downloadPivotes(tickers, interval):
    pivotes = dict()
    i = 1
...
def getDBData(ticker, interval):
    ...
    '''
    data = query.select(ticker=ticker, interval=interval)
    data.index = pd.to_datetime(data.index, utc=False)

    return data


def createTestingDf(tickers, interval, pivotes, smaValue):
    dfFinal = dict()
    i = 1
    for ticker in tickers:
        ...
        if testingMode == 'On':
            data = getDBData(ticker=ticker, interval=interval)
...
def main():
    ...
    # Pivot calculation
    pivotes = downloadPivotes(tickers=tickers, interval=pivotInterval)
